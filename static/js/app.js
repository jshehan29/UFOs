...
// Handle button click to filter table
function handleClick() {
    ...
    if (date) {
        // Apply 'filter' to the table data to only keep rows where 'datetime'
        // value matches the filter value
        filteredData = filteredData.filter(row => row.datetime === date);
    };

    // Build the filtered table
    // Note: If no date was entered, the table will just be the original table
